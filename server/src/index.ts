...
type Player = {
  ...
  lastShot:number; lastReceivedSeq:number; lastProcessedSeq:number; input:Input; slowUntil:number; burn:BurnState;
};
...
type Room = {
  ...
  winner?:string; winnerId?:string; returnAt?:number; tick:number; lastStep:number;
...
function publicPlayer(p:Player){const {input,lastShot,burn,lastReceivedSeq,lastProcessedSeq,...safe}=p;return safe;}
function roomState(room:Room){return{protocolVersion:MULTIPLAYER_PROTOCOL_VERSION,code:room.code,host:room.host,state:room.state,winner:room.winner,winnerId:room.winnerId,returnAt:room.returnAt,serverTime:now(),players:[...room.players.values()].map(publicPlayer),pickups:room.pickups};}
...
function beginRound(io:Server,room:Room){clearGameOverTimer(room);clearRespawns(room);room.state='PLAYING';room.winner=undefined;room.winnerId=undefined;room.history=[];room.pickups=createPickups();room.lastStep=now();[...room.players.values()].forEach((p,i)=>resetPlayer(p,i,true));emitRoom(io,room);}
...
function kill(io:Server,room:Room,attacker:Player,victim:Player){
  ...
  if(attacker.kills>=10){clearRespawns(room);clearGameOverTimer(room);room.state='GAME_OVER';room.winner=attacker.name;room.winnerId=attacker.id;room.returnAt=now()+10000;room.gameOverTimer=setTimeout(()=>returnToLobby(io,room),10000);emitRoom(io,room);return;}
...
function shoot(io:Server,room:Room,shooter:Player,clientTime:number){
  ...
  shooter.lastShot=time;state.ammo--;const base=direction(shooter.input.yaw,shooter.input.pitch),history=historicalPlayers(room,clientTime),hitIds=new Set<string>();
...
function step(io:Server,room:Room,dt:number){
  ...
  for(const p of room.players.values()){
    ...
    stepPlayer(p,{...p.input,lookX:0,lookY:0},dt*(p.slowUntil>time ? .65 : 1));
