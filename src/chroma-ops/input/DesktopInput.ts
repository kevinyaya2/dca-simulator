...
export class DesktopInput {
  private keys=new Set<string>(); private onKey=(e:KeyboardEvent)=>{ const k=e.key.toLowerCase(); if(['w','a','s','d',' ','r','q','1','2','3','4','5','6'].includes(k)) e.preventDefault(); if(e.type==='keydown') this.keys.add(k); else this.keys.delete(k); if(e.type==='keydown'){ if(k===' ') this.input.set({jump:true}); if(k==='r') this.input.set({reload:true}); if(/^[1-6]$/.test(k)) this.input.set({switchWeapon:+k}); }};
